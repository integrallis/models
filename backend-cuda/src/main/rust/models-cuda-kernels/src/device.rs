...
// Q6_K needs MAX_BLOCKS_PER_ROW i32 = 128 * 4 = 512 bytes, one exact sum per super-block.
// Q4_K needs MAX_BLOCKS_PER_ROW * 2 i32 = 1024 bytes and reuses the same array.
// Q4_K is therefore the binding case at 1024 bytes. This was 4096 while Q6_K kept eight float
// lane accumulators per block; collapsing that to one exact integer sum freed three quarters of
// it, and shared memory per block is what bounds how many blocks stay resident on an SM.
