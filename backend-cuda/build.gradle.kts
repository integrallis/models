...
// The PTX is a generated resource on the main source set, so the sources jar consumes it too.
// Without this Gradle fails the build on an undeclared dependency the first time the module is
// built from clean; it only appeared to work when the generating task happened to have run.
