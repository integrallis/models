...
def resolve_initial_adapter(
    ...
    """Verify an exact compatible adapter before continuing its training."""
    try:
        manifest = json.loads(training_manifest.read_text())
        config = json.loads((adapter / "adapter_config.json").read_text())
    except (OSError, json.JSONDecodeError) as error:
        raise ValueError(f"cannot read initial adapter provenance: {error}") from error
    if not isinstance(manifest, dict) or manifest.get("schemaVersion") != 1:
        raise ValueError("initial adapter training manifest must use schemaVersion 1")
    if manifest.get("kind") != "activated-lora-tool-specialist":
        raise ValueError("initial adapter training manifest has the wrong kind")
    if manifest.get("base") != {
        "model": expected_model,
        "revision": expected_revision,
    }:
        raise ValueError("initial adapter base identity differs from the selected candidate")
    declared_adapter = manifest.get("adapter")
    if not isinstance(declared_adapter, dict):
        raise ValueError("initial adapter manifest has no adapter metadata")
    if declared_adapter.get("rank") != expected_rank or declared_adapter.get("alpha") != expected_alpha:
        raise ValueError("initial adapter rank or alpha differs from the requested continuation")
    if declared_adapter.get("targetModules") != TARGET_MODULES:
        raise ValueError("initial adapter target modules differ from the training contract")
    if manifest.get("invocation", {}).get("tokens") != INVOCATION_TOKENS:
        raise ValueError("initial adapter invocation tokens differ from the training contract")
    if not isinstance(config, dict) or str(config.get("peft_type", "")).upper() != "LORA":
        raise ValueError("initial adapter configuration is not LoRA")
    if config.get("r") != expected_rank or config.get("lora_alpha") != expected_alpha:
        raise ValueError("initial adapter configuration rank or alpha differs")
    if config.get("alora_invocation_tokens") != INVOCATION_TOKENS:
        raise ValueError("initial adapter configuration invocation tokens differ")
    if sorted(config.get("target_modules", [])) != sorted(TARGET_MODULES):
        raise ValueError("initial adapter configuration target modules differ")

    adapter_sha256 = None
    declared_files = declared_adapter.get("files")
    if not isinstance(declared_files, list) or not declared_files:
        raise ValueError("initial adapter manifest has no declared files")
    for entry in declared_files:
        if not isinstance(entry, dict):
            raise ValueError("initial adapter file entry must be an object")
        name = entry.get("name")
        ...
        if (
            ...
            raise ValueError(f"initial adapter file declaration is invalid: {entry}")
        path = adapter / name
        if not path.is_file() or path.stat().st_size != expected_size:
            raise ValueError(f"initial adapter file size differs: {name}")
        actual_hash = sha256(path)
        if actual_hash != expected_hash:
            raise ValueError(
                f"initial adapter file SHA-256 differs for {name}: "
                f"expected {expected_hash}, got {actual_hash}"
            )
        if name == "adapter_model.safetensors":
            if adapter_sha256 is not None:
                raise ValueError("initial adapter declares multiple weight files")
            adapter_sha256 = actual_hash
    if adapter_sha256 is None:
        raise ValueError("initial adapter manifest has no adapter_model.safetensors")
    return {
        "adapterSha256": adapter_sha256,
        "trainingManifestSha256": sha256(training_manifest),
        "trainingManifest": str(training_manifest),
    }
...
def main() -> None:
    ...
    parser.add_argument("--initial-training-manifest", type=Path)
    ...
    if (args.initial_adapter is None) != (args.initial_training_manifest is None):
        parser.error("initial adapter and initial training manifest must be supplied together")
    initial_adapter = (
