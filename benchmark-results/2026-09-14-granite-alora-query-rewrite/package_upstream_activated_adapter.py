...
def require_token_ids(tokens: list[int]) -> list[int]:
    if not tokens or any(not isinstance(token, int) or isinstance(token, bool) or token < 0 for token in tokens):
        raise ValueError("invocation tokens must be a nonempty array of nonnegative integers")
    return list(tokens)
...
def package_upstream_adapter(
    *,
    ...
    tokens = require_token_ids(invocation_tokens)

    config_path = adapter_directory / "adapter_config.json"
    card_path = adapter_directory / "README.md"
    weights_path = adapter_directory / ADAPTER_WEIGHTS
    for path, name in ((config_path, "adapter configuration"), (card_path, "adapter model card"), (weights_path, "adapter weights")):
        if not path.is_file():
            raise ValueError(f"{name} does not exist: {path}")
    ...
    invocation = require_text(config.get("invocation_string"), "adapter_config.invocation_string")
