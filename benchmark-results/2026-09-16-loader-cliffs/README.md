...
| `native-gated-delta-net-unavailable` | `Qwen35ForwardPass.applyGatedDeltaNetRecurrence`, called for every GDN layer and token | `qwen35-native-gdn-false`: `-Dmodels.native.gatedDeltaNet=false` | 1 (the branch runs once per GDN layer for each of the 14 prompt and decode tokens) | `performance-cliff.native-gated-delta-net-unavailable=architecture=qwen35, kernel=rust-ffm-quantized-v13` | `qwen35-native-gdn-true`, `qwen35-java-baseline` (0 events in total each) |
