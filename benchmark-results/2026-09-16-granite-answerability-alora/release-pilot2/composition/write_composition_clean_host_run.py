...
write_clean_host_run.py) plus the facts only a composition run can establish:
  ...
  + modeljarsVersion, casesPerSuite, armOrder, suites, casesPerArm,
    controlMedianMillis, compositeMedianMillis, latencyImprovement, publishedArtifacts
...
* publishedArtifacts - copied from the program's own report, then re-checked: every member must
                   carry a group:artifact:version coordinate, a 64-hex artifact sha256, and both
                   resolvedFromCentral and runViaPublicApi exactly true. A member the runtime did
                   not resolve from a Maven-layout marker JAR fails the record, it does not
                   silently become a passing artifact.
...
* pass           - exitCode == 0, freshMachine, javaMajor == 25, the program report's own pass, the
                   measurement count the configuration implies, the artifact and median checks
                   above, and the output log's last non-empty line being exactly the program's PASS
                   verdict for every measurement.
...
def published_artifacts(report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for artifact in artifacts:
        ...
        if SHA256.fullmatch(artifact["sha256"]) is None:
            raise ValueError(f"{artifact['modelId']} sha256 must be a lowercase SHA-256")
...
def measurements_complete(report: dict[str, Any], expected: int) -> bool:
    ...
    for measurement in measurements:
        ...
        if measurement.get("pass") is not True:
            return False
        if arm == "composite" and measurement.get("physicallySharesPrefix") is not True:
            return False
        if arm == "control" and measurement.get("physicallySharesPrefix") is not False:
            return False
