...
__attribute__((target("avx2,fma,f16c"))) static double bench_avx2(int rows) {
  ...
  for (int round = 0; round < 3; round++) {
    ...
    for (int row = 0; row < rows; row++) {
      ...
      for (int block = 0; block < BLOCKS; block++) {
        ...
        // Nibble unpack is hoisted above the batch loop, exactly as the real kernel does it, so
        // its cost is amortised across BATCH rows and the dot dominates.
        for (int b = 0; b < BATCH; b++) {
          for (int g = 0; g < 8; g++) {
            __m256i a = _mm256_loadu_si256(
                (const __m256i *)(activations + b * COLS + block * QK_K + g * 32));
  ...
  (void)sink;
  double macs = (double)rows * COLS * BATCH;
      ...
      for (int block = 0; block < BLOCKS; block++) {
        ...
        for (int g = 0; g < 4; g++) {
          __m512i a = _mm512_loadu_si512(
              (const void *)(activations + b * COLS + block * QK_K + g * 64));
...
int main(void) {
  ...
  activations = aligned_alloc(64, (size_t)COLS * BATCH);
  for (size_t i = 0; i < weight_bytes; i++) weights[i] = (uint8_t)(i * 31 + 7);
  for (size_t i = 0; i < (size_t)COLS * BATCH; i++) activations[i] = (int8_t)(i % 71 - 35);
  printf("  Q4_K inner loop, real shape, BATCH %d: %d rows x %d cols, %.1f MiB weights\n\n",
         BATCH, ROWS / 8, COLS, weight_bytes / 1048576.0);
  double avx2 = bench_avx2(ROWS / 8);
  double avx512 = bench_avx512(ROWS / 8);
