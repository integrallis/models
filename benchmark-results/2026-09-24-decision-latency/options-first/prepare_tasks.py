...
Columns: id, type, labels, prompt, state, instructions, rubric-block. The last three are the
prompt's own parts, so an arm can reorder them instead of reparsing the flattened prompt.
"""
import json, sys
...
with open(dst, "w") as out:
    for line in open(src):
        ...
        lines.append("Answer:")
        prompt = "\n".join(lines)
        # The rubric block and the instructions are emitted separately as well, so an arm can
        # rebuild the prompt in a different order without reparsing a flattened one. Extra columns
        # only; every reader that wants the four it already had is unaffected.
        rubric_block = "\n".join(["Options:"] + [f"- {lab}: {rubric.get(lab, lab)}" for lab in labels])
        def flat(text):
            return text.replace("\t", " ").replace("\n", "\\n")
        out.write("\t".join([t["id"], q["type"], "|".join(labels),
                             flat(prompt), flat(state_text), flat(q["instructions"]),
                             flat(rubric_block)]) + "\n")
        n += 1
