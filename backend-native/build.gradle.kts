...
// backend-native - Models-owned Rust inference kernels exposed through Java FFM
...
val privateTestBundleJar =
    aggregateNativeArtifacts?.let { nativeArtifacts ->
        tasks.register<Jar>("privateTestBundleJar") {
            ...
            archiveBaseName.set("backend-native-standalone")
            destinationDirectory.set(layout.buildDirectory.dir("native-artifacts"))
...
if (aggregateNativeArtifacts != null) {
    val nativeArtifacts = aggregateNativeArtifacts
    val nativeVerification =
        verifyNativePublicationArtifacts
            ?: error("Native artifact verification was not configured")
    tasks.named<Jar>("jar") {
        dependsOn(nativeVerification)
        inputs.files(nativeArtifacts.values)
        duplicatesStrategy = DuplicatesStrategy.FAIL
        nativeArtifacts.values.forEach { artifact ->
            from(zipTree(artifact)) {
                include("META-INF/models/native/**")
            }
        }
        manifest {
            attributes(
        ...
        tasks.register("verifyPrivateTestBundle") {
            ...
            doLast {
                JarFile(bundleJar.get().archiveFile.get().asFile).use { jar ->
                    ...
                    require(jar.getJarEntry("org/modeljars/ModelJarRegistry.class") == null) {
                        "Standalone native backend must not bundle the optional ModelJars registry"
                    }
    ...
    configure<PublishingExtension> {
        publications {
            create<MavenPublication>("githubStandalone") {
                artifactId = "backend-native-standalone"
                artifact(bundleJar)
                pom {
