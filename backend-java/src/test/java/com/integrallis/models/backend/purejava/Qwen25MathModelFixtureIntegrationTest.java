...
@Tag("integration")
class Qwen25MathModelFixtureIntegrationTest {

  private static final int INTEGRATION_CONTEXT_LENGTH = 128;

  private static final ModelFixtureRequirement QWEN25_MATH_1_5B_Q4_K_M =
      ModelFixtureRequirement.of("hf://bartowski/Qwen2.5-Math-1.5B-Instruct-GGUF")
          .version("[2.5.0,3.0.0)")
          .variant("q4_k_m")
          .backend("rust-ffm")
          .capability("math");
  ...
  private static ModelFixtureDescriptor descriptorWithInstalledArtifact() {
    ModelFixtureDescriptor descriptor =
        ModelFixtureRegistry.fromClasspath().resolve(QWEN25_MATH_1_5B_Q4_K_M).orElseThrow();
