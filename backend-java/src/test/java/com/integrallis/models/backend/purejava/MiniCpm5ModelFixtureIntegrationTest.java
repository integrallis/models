...
@Tag("integration")
class MiniCpm5ModelFixtureIntegrationTest {
  ...
  private static final ModelFixtureRequirement MINICPM5_1B_Q4_K_M =
  ...
  void mixedKProjectionGroupingMatchesTheTwoDispatchControlExactly() {
    ...
    try {
      float[] expected;
      int[] promptTokens;
      try (PureJavaBackend control = PureJavaBackend.load(descriptor.localPath().orElseThrow())) {
      ...
      try (PureJavaBackend grouped = PureJavaBackend.load(descriptor.localPath().orElseThrow())) {
  ...
  void batchedPrefillMatchesSequentialInferenceExactly() {
    ...
    try (PureJavaBackend sequential = PureJavaBackend.load(descriptor.localPath().orElseThrow())) {
  ...
  private static ModelFixtureDescriptor descriptorWithInstalledArtifact() {
    ModelFixtureDescriptor descriptor =
        ModelFixtureRegistry.fromClasspath().resolve(MINICPM5_1B_Q4_K_M).orElseThrow();
