...
@Tag("slow")
class Qwen3LargeModelFixtureSlowTest {

  private static final int INTEGRATION_CONTEXT_LENGTH = 128;

  private static final ModelFixtureRequirement QWEN3_8B_Q4_K_M =
      ModelFixtureRequirement.of("hf://Qwen/Qwen3-8B-GGUF")
          .version("[3.0.0,4.0.0)")
          .variant("q4_k_m")
          .backend("pure-java")
          .capability("chat");
  ...
  private static ModelFixtureDescriptor descriptorWithInstalledArtifact() {
    ModelFixtureDescriptor descriptor =
        ModelFixtureRegistry.fromClasspath().resolve(QWEN3_8B_Q4_K_M).orElseThrow();
