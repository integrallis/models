...
@Tag("slow")
class DeepSeekCoderLargeModelFixtureSlowTest {

  private static final int INTEGRATION_CONTEXT_LENGTH = 128;

  private static final ModelFixtureRequirement DEEPSEEK_CODER_6_7B_Q4_K_M =
      ModelFixtureRequirement.of("hf://TheBloke/deepseek-coder-6.7B-instruct-GGUF")
          .version("[6.7.0,7.0.0)")
          .variant("q4_k_m")
          .backend("pure-java")
  ...
  private static ModelFixtureDescriptor descriptorWithInstalledArtifact() {
    ModelFixtureDescriptor descriptor =
        ModelFixtureRegistry.fromClasspath().resolve(DEEPSEEK_CODER_6_7B_Q4_K_M).orElseThrow();
    assertThat(Files.exists(descriptor.localPath().orElseThrow()))
        .as(
            "%s must be present. Run :backend-java:slowTest or the fixture"
