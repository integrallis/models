...
@Tag("slow")
class SmolLm3ModelFixtureSlowTest {

  private static final int INTEGRATION_CONTEXT_LENGTH = 128;

  private static final ModelFixtureRequirement SMOLLM3_3B_Q4_K_M =
      ModelFixtureRequirement.of("hf://ggml-org/SmolLM3-3B-GGUF")
          .version("[3.0.0,4.0.0)")
          .variant("q4_k_m")
          .backend("pure-java")
  ...
  private static ModelFixtureDescriptor descriptorWithInstalledArtifact() {
    ModelFixtureDescriptor descriptor =
        ModelFixtureRegistry.fromClasspath().resolve(SMOLLM3_3B_Q4_K_M).orElseThrow();
    assertThat(Files.exists(descriptor.localPath().orElseThrow()))
        .as(
            "%s must be present. Run :backend-java:smolLm33BSlowTest.",
