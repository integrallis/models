...
@Tag("integration")
class DeepSeekCoderModelFixtureIntegrationTest {

  private static final int INTEGRATION_CONTEXT_LENGTH = 128;

  private static final ModelFixtureRequirement DEEPSEEK_CODER_1_3B_Q4_K_M =
      ModelFixtureRequirement.of("hf://TheBloke/deepseek-coder-1.3b-instruct-GGUF")
          .version("[1.3.0,2.0.0)")
          .variant("q4_k_m")
          .backend("pure-java")
  ...
  void matchesLlamaCppGreedyCodeCompletionTokens() {
    ...
    try (PureJavaBackend backend = PureJavaBackend.load(descriptor.localPath().orElseThrow())) {
      ...
      int[] promptTokens = backend.tokenizer().encode("def fibonacci(n):");
      assertThat(promptTokens).containsExactly(1551, 12606, 249, 305, 2711, 7, 77, 1772);
      assertThat(greedyTokens(backend, new int[] {2494}, 1))
          .as("single-token greedy ID must match llama.cpp b9960 for the pinned GGUF")
          .containsExactly(185);
  ...
  private static ModelFixtureDescriptor descriptorWithInstalledArtifact() {
    ModelFixtureDescriptor descriptor =
        ModelFixtureRegistry.fromClasspath().resolve(DEEPSEEK_CODER_1_3B_Q4_K_M).orElseThrow();
    assertThat(Files.exists(descriptor.localPath().orElseThrow()))
        .as(
            "%s must be present. Run :backend-java:integrationTest or the fixture"
