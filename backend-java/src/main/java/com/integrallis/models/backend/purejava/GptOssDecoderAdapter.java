...
/** Adapts GPT-OSS sequence state to the architecture-neutral backend contract. */
final class GptOssDecoderAdapter implements PureJavaDecoder {

  private final class GptOssSession implements Session {
    private final GptOssForwardPass.Session delegate = forwardPass.openSession();
  ...
  private final GptOssForwardPass forwardPass;
  private final GptOssSession defaultSession;

  GptOssDecoderAdapter(GptOssForwardPass forwardPass) {
    this.forwardPass = Objects.requireNonNull(forwardPass, "forwardPass");
    defaultSession = new GptOssSession();
  }
  ...
  public LogitBatch forwardBatchTransient(Session[] sessions, int[] tokens) {
    ...
    if (sessions.length != 1 || tokens.length != 1) {
      throw new IllegalArgumentException("GPT-OSS currently supports one session per batch");
    }
    GptOssSession session = requireSession(sessions[0]);
    float[] logits =
  ...
  private float[] prefill(GptOssForwardPass.Session session, int[] tokens, int startPosition) {
  ...
  private static void validate(GptOssForwardPass.Session session, int[] tokens, int startPosition) {
    Objects.requireNonNull(tokens, "tokens");
    if (tokens.length == 0) {
  ...
  private GptOssSession requireSession(Session session) {
    Objects.requireNonNull(session, "session");
    if (!(session instanceof GptOssDecoderAdapter.GptOssSession gptOssSession)) {
      throw new IllegalArgumentException("session belongs to a different decoder");
    }
    return gptOssSession;
  }
}
