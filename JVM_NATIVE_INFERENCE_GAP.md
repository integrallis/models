...
| Development launchers can silently suppress the optimizing compiler required by inference. | The [Spring Boot reproduction](benchmark-results/spring-boot-launch-20260904/README.md) shows the same Qwen Spring AI application completing from an ordinary launch while `-XX:TieredStopAtLevel=1` leaves every GGUF worker active in unoptimized Vector API code. | Runtime and framework tooling should make optimization-level requirements observable. Libraries must detect incompatible launch flags before expensive model work; development launchers should make their C1-only tradeoff visible and easy to disable per application. |
