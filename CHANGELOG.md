...
- NEON kernels for AArch64 (Apple Silicon, AWS Graviton 2 and later, Ampere). The Q4_0, Q5_0, Q8_0, Q4_K and Q6_K matrix kernels now use `sdot` when the CPU has the dot-product extension; before this, these CPUs ran the scalar fallback. Only the exact integer arithmetic is vectorized and every floating-point step keeps the scalar order, so results are bit-identical to the scalar kernels, which new tests check for single rows and batched row ranges. Gemma 3 1B Q4_K_M on an Apple M5 Pro with `models.native.quantizedDecode=true`: decode 17.8 → 48.5 tokens/s and prefill 35 → 165 tokens/s, with identical output. `JMODELS_KERNELS_DISABLE_NEON=1` selects the scalar kernels for comparisons.
