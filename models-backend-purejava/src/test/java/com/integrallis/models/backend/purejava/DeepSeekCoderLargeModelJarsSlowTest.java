...
@Tag("slow")
class DeepSeekCoderLargeModelJarsSlowTest {

  private static final int INTEGRATION_CONTEXT_LENGTH = 128;

  private static final ModelJarRequirement DEEPSEEK_CODER_6_7B_Q4_K_M =
      ModelJarRequirement.forSource("hf://TheBloke/deepseek-coder-6.7B-instruct-GGUF")
  ...
  void matchesLlamaCppGreedyCodeCompletionTokens() {
    ...
    try (PureJavaBackend backend = PureJavaBackend.load(descriptor)) {
      ...
      assertThat(greedyTokens(backend, promptTokens, 4))
          .as("greedy token IDs must match llama.cpp b9960 for the pinned 6.7B GGUF")
  ...
  private static ModelJarDescriptor descriptorWithInstalledArtifact() {
    ModelJarDescriptor descriptor =
        ModelJarRegistry.fromClasspath().resolve(DEEPSEEK_CODER_6_7B_Q4_K_M).orElseThrow();
    assertThat(Files.exists(descriptor.localPath().orElseThrow()))
        .as(
            "%s must be present. Run :models-backend-purejava:slowTest or the fixture"
