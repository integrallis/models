...
projection, reduces the per-position hidden states to one vector, and scales
...
choice is a property of how the model was trained.
...
Retrieval quality is a published property of the weights. What a runtime can get
wrong is pooling, rotary embeddings, dequantization, and normalization, so the
gate measures whether it reproduces a reference implementation.

`models-bench` embeds a pinned probe set and compares the vectors against ones a
pinned llama.cpp build produced from the same model bytes:
...
a normalized reference at exactly 1.0. Vector length is therefore checked
separately.

The gate refuses to run when the probe-set or artifact digest has moved, so a
stale reference fails rather than comparing different inputs.
