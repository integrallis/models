...
Both adapters own the backend they wrap and close it. The Spring AI constructor
that accepts an `ObservationRegistry` emits the framework's standard embedding
observations with the supplied model identity. To store the vectors, see
