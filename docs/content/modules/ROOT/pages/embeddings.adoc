...
BERT/MiniLM GGUF; Models reads `bert.pooling_type` and rejects an override that
would silently change the trained representation.
...
and ordinary prose. Dedicated encoders additionally pin WordPiece token IDs and
the complete pooled vector. The gate takes the worst probe.
