...
ssh -o ConnectTimeout=20 root@$BOX '/root/jdk/bin/java --add-modules jdk.incubator.vector \
  --enable-native-access=ALL-UNNAMED -cp "/root/demo/demo-dist/*" \
  com.integrallis.models.decisions.BriefingDemo \
  /root/demo/squad2-noul-v0.1.idsn /root/models/granite.gguf \
  /root/demo/document.txt /root/demo/questions.txt /root/demo/decisions-timing.json' 2>/dev/null \
  | grep -vE "^WARNING|^INFO|^$"
scp -q -o ConnectTimeout=20 root@$BOX:/root/demo/decisions-timing.json /tmp/demo/decisions-timing.json

rule
python3 /tmp/demo/compare.py /tmp/demo/jev-timing.json /tmp/demo/decisions-timing.json "$GOLD"
